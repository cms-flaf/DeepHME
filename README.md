...
All arguments except for `output_format` default to `None`. 
Mandatory arguments are `pt`, `eta`, `phi`, `mass` of various objects and `event_id`. If they are not provided, an exceprion is raised. 
It is allowed to pass `None` for jet and fatjet tagger scores (e.g. if they are not available in the NanoAOD version being used). These are checked if they exist among model's inputs, and if they were, but value `None` was provided, an exception will be raised.
It is also allowed to leave `lep2*` variables as `None` if `channel` set to `SL`. Usage example is 
```
